...
describe('Auth Controller Tests', () => {
  ...
  describe('register', () => {
    describe('Succès', () => {
      it('enregistre un nouvel utilisateur avec tous les champs', async () => {
    ...
    describe('Erreurs de base de données', () => {
      ...
      ['connection', 'timeout', 'sql', 'validation'].forEach(errorType => {
        it(`retourne 500 pour erreur ${errorType}`, async () => {
          ...
          expect(logger.error).toHaveBeenCalledWith('Registration error:', errors[errorType]);
      ...
      it('inclut les détails d\'erreur en développement', async () => {
        const { mockReq, mockRes, mockStatus, mockJson } = createMockReqRes();
        mockReq.body = { name: 'testuser', password: 'password123' };
    ...
    describe('Erreurs de base de données', () => {
      ...
      ['connection', 'timeout', 'sql'].forEach(errorType => {
        it(`retourne 500 pour erreur ${errorType}`, async () => {
          ...
          User.findOne = vi.fn().mockRejectedValue(errors[errorType]);

          await login(mockReq, mockRes);

          expect(logger.error).toHaveBeenCalledWith('Login error:', errors[errorType]);
          expect(mockStatus).toHaveBeenCalledWith(500);
          expect(mockJson).toHaveBeenCalledWith({ 
            error: "Login failed"
          });
  ...
  describe('refreshToken', () => {
    ...
    describe('Erreurs de validation', () => {
      it('retourne 401 si le refresh token est manquant', async () => {
        const { mockReq, mockRes, mockStatus, mockJson } = createMockReqRes();
        mockReq.body = {};
    ...
    describe('Erreurs d\'authentification', () => {
      ...
      it('retourne 403 si le refresh token ne correspond pas', async () => {
        const { mockReq, mockRes, mockStatus, mockJson } = createMockReqRes();
  ...
  describe('logout', () => {
    describe('Succès', () => {
      it('déconnecte l\'utilisateur en supprimant le refresh token', async () => {
    ...
    describe('Erreurs de token', () => {
      it('retourne 500 si le token JWT est invalide', async () => {
        const { mockReq, mockRes, mockStatus, mockJson } = createMockReqRes();
        mockReq.body = { refreshToken: 'invalid.token' };
    ...
    describe('Erreurs de base de données', () => {
      ...
      ['connection', 'timeout', 'sql'].forEach(errorType => {
        it(`gère l'erreur ${errorType}`, async () => {
          ...
          expect(logger.error).toHaveBeenCalledWith('Logout error:', errors[errorType]);
          expect(mockStatus).toHaveBeenCalledWith(500);
          expect(mockJson).toHaveBeenCalledWith({ error: "Internal server error" });
        });
      });
